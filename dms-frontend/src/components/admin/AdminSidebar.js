...
const AdminSidebar = ({ activeTab, setActiveTab, sidebarOpen, toggleSidebar, adminName, adminProfilePic }) => {
  ...
  return (
    <>
      <aside className={`fixed top-0 bottom-0 left-0 z-[200] w-64 bg-[#001f3f] text-white p-6 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.3)] transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] border-r border-white/5 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        {/* Brand Header */}
        <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-6 cursor-pointer hover:opacity-90 transition-opacity" onClick={() => setActiveTab('profile')}>
          <div className="flex items-center gap-3">
            {adminProfilePic ? (
              <div className="w-11 h-11 bg-white rounded-full flex items-center justify-center shadow-lg shadow-black/20 overflow-hidden shrink-0 border-2 border-[#d4a017]">
                <img src={`${process.env.REACT_APP_API_URL}${adminProfilePic}`} alt="Profile" className="w-full h-full object-cover" />
              </div>
            ) : (
              <div className="w-11 h-11 bg-white rounded-full flex items-center justify-center shadow-lg shadow-black/20 shrink-0 border-2 border-[#d4a017] text-[#001f3f] font-black text-xl">
                {adminName ? adminName.replace(/admin/ig, '').trim().charAt(0).toUpperCase() || 'A' : 'A'}
              </div>
            )}
            <div>
              <h1 className="font-black text-sm tracking-wider text-white uppercase line-clamp-1" title={adminName ? adminName.replace(/admin/ig, '').trim() : 'SUPER'}>
                {adminName ? adminName.replace(/admin/ig, '').trim() : 'SUPER'}
              </h1>
