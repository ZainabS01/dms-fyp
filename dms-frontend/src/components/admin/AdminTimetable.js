...
const AdminTimetable = () => {
    ...
    const teacherTimetables = adminTimetables.filter(t => t.audience === 'teacher');
    const studentTimetables = adminTimetables.filter(t => t.audience === 'student');

    const renderTimetableRow = (t) => (
    ...
    return (
        <div className="w-full -mt-2 sm:-mt-4 animate-fadeIn pb-[10vh] px-2 sm:px-4 md:px-6">
            <ToastContainer />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl">
                {/* 1. UPLOAD */}
                <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100 border-t-8 border-[#001f3f] h-fit">
                    <h3 className="font-black text-[#001f3f] uppercase text-sm mb-4">UPLOAD TIMETABLE</h3>
                    
                    <input type="text" placeholder="Timetable Title" className="w-full border-2 border-slate-100 bg-slate-50 p-3 mb-3 rounded-lg outline-none focus:border-[#001f3f] font-bold text-xs" value={uploadData.title} onChange={(e) => setUploadData({...uploadData, title: e.target.value})} />
                    ...
                    <select 
                        ...
                        className="w-full border-2 border-slate-100 bg-slate-50 p-3 mb-3 rounded-lg outline-none focus:border-[#001f3f] font-bold text-xs cursor-pointer pr-10"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                        <input type="date" className="border-2 border-slate-100 bg-slate-50 p-3 rounded-lg outline-none focus:border-[#001f3f] font-bold text-xs" onChange={(e) => setUploadData({...uploadData, startDate: e.target.value})} />
                        <input type="date" className="border-2 border-slate-100 bg-slate-50 p-3 rounded-lg outline-none focus:border-[#001f3f] font-bold text-xs" onChange={(e) => setUploadData({...uploadData, endDate: e.target.value})} />
                    </div>
                    
                    <input type="file" className="w-full mb-4 text-xs font-semibold text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" onChange={(e) => setUploadData({...uploadData, file: e.target.files[0]})} />
                    <button onClick={handleUpload} className="w-full bg-[#001f3f] text-[#d4a017] py-3.5 rounded-lg font-black uppercase text-xs tracking-wider shadow-lg hover:opacity-90 transition-all">UPLOAD NOW</button>
                </div>

                {/* 2. ADMIN UPLOADED OVERVIEW */}
                <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100 border-t-8 border-[#d4a017] h-fit">
                    <h3 className="font-black text-[#001f3f] uppercase text-sm mb-4">HISTORY</h3>
                    <div className="space-y-6">
                        
                        {/* TEACHER TIMETABLES */}
                        <div>
                            <div className="bg-slate-50 border-l-4 border-[#001f3f] px-3 py-2 mb-3">
                                <h4 className="font-black text-[#001f3f] text-xs tracking-wider">TEACHER UPLOADS</h4>
                            </div>
                            {teacherTimetables.length > 0 ? teacherTimetables.map(renderTimetableRow) : <p className="text-gray-400 text-[10px] font-bold px-2">No timetables for teachers.</p>}
                        </div>

                        {/* STUDENT TIMETABLES */}
                        <div>
                            <div className="bg-slate-50 border-l-4 border-[#d4a017] px-3 py-2 mb-3">
                                <h4 className="font-black text-[#d4a017] text-xs tracking-wider">STUDENT UPLOADS</h4>
                            </div>
                            {studentTimetables.length > 0 ? studentTimetables.map(renderTimetableRow) : <p className="text-gray-400 text-[10px] font-bold px-2">No timetables for students.</p>}
                        </div>
