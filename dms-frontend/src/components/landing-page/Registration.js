...
const Registration = () => {
  ...
  // Setup screen ko bypass rakhne ke liye isey hamesha false rakhein
  ...
  const handleRegister = async (e) => {
    ...
    const userData = {
      ...
      department,
      rollNo: userRole.toLowerCase() === 'student' ? rollNo : undefined,
    ...
    try {
      ...
      if (response.data.success) {
        if (userRole === 'Teacher') {
          // Alert ya toast dikha kar seedha login par bhejien
          toast.success(`Registration Successful! PIN sent to ${email}`);
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#001f3f] flex items-center justify-center pt-[200px] pb-24 px-6 overflow-x-hidden">
      ...
      >
        ...
        {/* Form Section - Layout Kept Intact */}
                        ...
                        {userRole === 'Student' && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <input type="number" placeholder="6 DIGITS" value={rollNo} onChange={handleRollChange} className="w-full p-3.5 mt-1 border-2 border-[#001f3f] rounded-2xl font-bold text-sm outline-none" required />
                            </div>
                            <div>
