...
const StudentHeader = ({ activePage, userName, studentProfilePic, onOpenNexi, setActiveTab, toggleSidebar }) => {
  const baseTitle = activePage ? activePage.replace('-', ' ') : 'Dashboard';
  const displayTitle = baseTitle.toLowerCase() === 'overview' ? 'Profile' : baseTitle;
  ...
  return (
    <header className="h-16 sm:h-20 lg:h-24 bg-white border-b border-slate-100 flex items-center justify-between px-2 sm:px-6 lg:px-10 sticky top-0 z-[100] shadow-sm">
      ...
      <div className="flex items-center gap-1.5 sm:gap-4 ml-1 md:ml-0 flex-shrink-0">
        ...
        {activePage && activePage.toLowerCase() !== 'overview' && activePage.toLowerCase() !== 'dashboard' && (
          <button 
